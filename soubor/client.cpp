...
void handleFile(int socket){
    while(1){
        ...
        char buf_ack[4];
        int len_ack = read(socket, buf_ack, 4);
        if (len_ack < 0){
        ...
        // char buf_file[fileStat.st_size];
        // int len_file = read(fd, buf_file, fileStat.st_size);
        // if (len_file < 0){
        //     log_msg(LOG_ERROR, "Cannot read from socket! \n");
        //     exit(0);
        // }
        char buf_file[16];
        sprintf(buf_file, "%d", fd);

        write(socket, buf_file, strlen(buf_file));
