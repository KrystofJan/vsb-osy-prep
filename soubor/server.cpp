...
int main( int t_narg, char **t_args )
{
    ...
    while ( 1 )
    {
        ...
        allClients.push_back(std::thread(handleClient, l_sock_client));
...
void handleClient(int socket){
    while(1){
        ...
        int file_length;
        sscanf(buf, "%d", &file_length);

        write(socket, ACK, strlen(ACK));

        char buf_file[file_length];
        int len_file = read(socket, buf_file, file_length);
        ...
        int file_fd;
        sscanf(buf_file, "%d", &file_fd);

        char buf_file_raw[file_length];
        int read_fd = read(file_fd, buf_file_raw, sizeof(buf_file_raw));
        if(read_fd < 0) {
            exit(0);
        }
