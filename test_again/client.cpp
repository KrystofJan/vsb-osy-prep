...
void handle(int socket){
    ...
    if (fd < 0) {
        log_msg( LOG_ERROR, "File is wrong!");
        exit( EXIT_FAILURE );
    }

    while (true){
        char socket_buf[BATCH_SIZE];
        int socket_len = read(socket, socket_buf, sizeof(socket_buf));
        if(socket_len < 0) {
            log_msg( LOG_ERROR, "File is wrong!");
            exit( EXIT_FAILURE );
        }
        socket_buf[socket_len] = 0;
        if(!strcasecmp(socket_buf, EOF)){
            break;
        }

        write(fd, socket_buf, socket_len );

        
    }
    log_msg(LOG_INFO, "ended");
