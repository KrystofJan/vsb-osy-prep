...
{   
    ...
    int len_stdin = read(STDIN_FILENO, buf_stdin, sizeof(buf_stdin) );

    if (len_stdin < 0)
        log_msg(LOG_ERROR, "Nejde cist stdin! \n");


    write(socket, buf_stdin, strlen(buf_stdin));

    int len = read(socket, buf, sizeof(buf) );

    if (len < 0)
        log_msg(LOG_ERROR, "Nejde cist! \n");
